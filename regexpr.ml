...
type regexp = 
  ...
  (*acc: regexp -> char list (string) -> (k: char list -> bool) -> bool *)
  (*k is the continuation; it looks at the rest of the string once we've checked the first part;
  start with checking if list is empty*)
let rec acc r clist k : bool = match r , clist with 
  | Zero          , s     -> false
  | One           , s     -> k s (*we do not consume anything & the remaining list is still s*)
  | Char c        , []    -> false 
  | Char c        , c1::s -> c = c1 && k s
  | Times(r1, r2) , s     -> acc r1 s (fun s' -> acc r2 s' k) (*consume part of s to match r1 & use
                                                                  the rest (s') to match r2*)
  | Plus(r1, r2)  , s     -> acc r1 s k || acc r2 s k
  | Star r        , s     ->  
     k s || acc r s (fun s' -> not(s = s') && acc (Star r) s' k)
     (*either it's empty and we don't consume anything OR we consume part of s to match r and evalute the rest (s')
        we want the rest to not be the same as s && to match s' with Star r*)
